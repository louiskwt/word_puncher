...
class Word_Puncher:
    ...
    def generate_exercise_with_answer_keys(self, lines, selected_words):
        ...
        for index in range(len(lines)):
            ...
            for word in inline_word_lst:
                if self.strip_punctuations(word) in selected_words[index]:
                    punctuations_in_word = self.find_punctuation(word)
                    processed_word_lst.append(punctuations_in_word['front'] + self.replace_word_with_blank(self.strip_punctuations(word)) + punctuations_in_word['end'])
                else:
                    processed_word_lst.append(word)
            lines[index] = " ".join(processed_word_lst)
        return [lines, self.format_answer_key([word for sublist in selected_words for word in sublist])]
    ...
    def extract_words(self, text):
        ...
        """
        return [[self.strip_punctuations(word) for word in line.split(" ")] for line in text]


    def find_random_words(self, word_lst, word_len=1, linked=True):
        ...
        while count < word_len:
            ...
            in_common_words = self.check_for_common_words(self.strip_punctuations(target_word))
    ...
    def replace_word_with_blank(self, word):
        ...
        '(______!)'
        """
        stripped_word = self.strip_punctuations(word)
        punctuations = self.find_punctuation(word) 
        if stripped_word != word:
            return punctuations['front'] + ('_' * len(stripped_word) * 2) + punctuations['end']
        else:
            return "_" * len(word) * 2

    def strip_punctuations(self, word):
        """
    ...
    def check_for_common_words(self, word):
        """
